...
# Filter data for the "localization Prediction" task
localization_data = data[data["task"] == "localization"]

# Further filter for cross-validation data split
localization_cv_data = localization_data[
    localization_data["data_split"] == "cross-validation"
]
# Group by model and metric to summarize the performance across folds
localization_summary = (
    localization_cv_data.groupby(["model", "metric"])["value"]
    .agg(["mean", "std"])
    .reset_index()
)
...
localization_summary_filtered = localization_summary[
    localization_summary["metric"].isin(key_metrics)
]
...
# Process data and compute p-values for the "localization Prediction" task
localization_results, localization_p_values = process_and_compare_data(
    localization_cv_data, "localization", key_metrics
)
...
for idx, metric in enumerate(key_metrics):
    ...
    subset = localization_results[localization_results["metric"] == metric_name]
    ...
    ax.text(
        ...
        f"p-value: {localization_p_values.get(metric_name, np.nan)}",
        ha="center",
        va="bottom",
        fontsize=12,
    )
...
# plt.suptitle("localization Prediction Performance Comparison", fontsize=16)
plt.tight_layout(rect=[0, 0.03, 1, 0.95])
# plt.show()

plt.savefig("./plots/plot3.2.3_loc.png", bbox_inches="tight", dpi=300)
plt.close()
...
localization_latex_table = localization_results.pivot(
    index="metric", columns="model", values=["Mean ± Std", "P-Value"]
).reset_index()
# Flatten the multi-level column index
localization_latex_table.columns = [
    " ".join(col).strip() for col in localization_latex_table.columns.values
]

# Print current column names to verify the structure
print("Current column names:", localization_latex_table.columns.tolist())
...
localization_latex_table.columns = new_column_names

# Initialize a list to store the p-values for each metric
p_values = []

# Add p-values to the localization_results DataFrame
for metric in key_metrics:
    metric_name = metric.replace("eval_", "")
    p_value = compute_p_value(localization_cv_data, "localization", metric)
    # Format the p-value with one decimal in scientific notation
    p_value_formatted = np.format_float_scientific(p_value, precision=1)
    localization_results.loc[
        localization_results["metric"] == metric_name, "P-Value"
    ] = p_value_formatted

localization_results["metric"] = localization_results["metric"].apply(
    lambda x: "eval_" + x if not x.startswith("eval_") else x
)


# Ensure the DataFrame is sorted by the metric order before pivoting
localization_results["metric_order"] = localization_results["metric"].map(
    lambda x: key_metrics.index(x)
)
localization_results.sort_values(by="metric_order", inplace=True)

# Pivot the table to get the desired layout
localization_latex_table = localization_results.pivot_table(
    index="metric",
    columns="model",
...
localization_latex_table = localization_latex_table.set_index("metric")
localization_latex_table = localization_latex_table.reindex(
    key_metrics
)  # Reorder based on key_metrics
localization_latex_table.reset_index(inplace=True)
...
for metric in key_metrics:
    ...
    for model in ["Ankh", "TooT-PLM-P2S"]:
        mean_std = localization_results[
            (localization_results["metric"] == metric)
            & (localization_results["model"] == model)
        ]["Mean ± Std"].values[0]
...
for metric in key_metrics:
    metric_name = metric.replace("eval_", "")
    p_value = localization_p_values[metric_name]
