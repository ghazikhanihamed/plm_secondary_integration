...
# Using the accelerator's prepare method for models and datasets
toot_plm_p2s_model = accelerator.prepare(toot_plm_p2s_model)
ankh_large_model = accelerator.prepare(ankh_large_model)

train_dataset = pd.read_csv("./dataset/ionchannels_membraneproteins_train.csv")
test_dataset = pd.read_csv("./dataset/ionchannels_membraneproteins_test.csv")
...
def get_embeddings(model, tokenizer, protein_sequences):
    ...
    outputs = tokenizer.batch_encode_plus(protein_sequences,
                                          ...
                                          return_tensors="pt")
    outputs = accelerator.prepare(outputs)
    with torch.no_grad():
...
accelerator.print(f"Accuracy for toot_plm_p2s: {accuracy_toot}")
accelerator.print(f"Accuracy for ankh_large: {accuracy_ankh}")

accelerator.print(f"F1 score for toot_plm_p2s: {f1_toot}")
accelerator.print(f"F1 score for ankh_large: {f1_ankh}")

accelerator.print(f"MCC for toot_plm_p2s: {mcc_toot}")
accelerator.print(f"MCC for ankh_large: {mcc_ankh}")
