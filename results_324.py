...
# Filter data for the "ionchannels Prediction" task
ionchannels_data = data[data["task"] == "ionchannels"]

# Further filter for cross-validation data split
ionchannels_cv_data = ionchannels_data[
    ionchannels_data["data_split"] == "cross-validation"
]
# Group by model and metric to summarize the performance across folds
ionchannels_summary = (
    ionchannels_cv_data.groupby(["model", "metric"])["value"]
    .agg(["mean", "std"])
    .reset_index()
)
...
ionchannels_summary_filtered = ionchannels_summary[
    ionchannels_summary["metric"].isin(key_metrics)
]
...
# Process data and compute p-values for the "ionchannels Prediction" task
ionchannels_results, ionchannels_p_values = process_and_compare_data(
    ionchannels_cv_data, "ionchannels", key_metrics
)
...
for idx, metric in enumerate(key_metrics):
    ...
    subset = ionchannels_results[ionchannels_results["metric"] == metric_name]
    ...
    ax.text(
        0,
        highest_bar - 0.07,  # Add an offset
        f"p-value: {ionchannels_p_values.get(metric_name, np.nan)}",
        ha="center",
        va="bottom",
        fontsize=12,
    )
...
# plt.suptitle("ionchannels Prediction Performance Comparison", fontsize=16)
plt.tight_layout(rect=[0, 0.03, 1, 0.95])
# plt.show()

plt.savefig("./plots/plot3.2.4_ion.png", bbox_inches="tight", dpi=300)
plt.close()


# Preparing data for LaTeX table
ionchannels_results["Mean ± Std"] = ionchannels_results.apply(
    lambda row: f"{row['mean']:.4f} ± {row['std']:.4f}", axis=1
)
ionchannels_results["P-Value"] = ionchannels_results["metric"].map(ionchannels_p_values)

# Generating LaTeX table
ionchannels_latex_table = ionchannels_results.pivot(
    index="metric", columns="model", values=["Mean ± Std", "P-Value"]
).reset_index()
# Flatten the multi-level column index
ionchannels_latex_table.columns = [
    " ".join(col).strip() for col in ionchannels_latex_table.columns.values
]

# Print current column names to verify the structure
print("Current column names:", ionchannels_latex_table.columns.tolist())
...
ionchannels_latex_table.columns = new_column_names

# Initialize a list to store the p-values for each metric
p_values = []

# Add p-values to the ionchannels_results DataFrame
for metric in key_metrics:
    metric_name = metric.replace("eval_", "")
    p_value = compute_p_value(ionchannels_cv_data, "ionchannels", metric)
    # Format the p-value with one decimal in scientific notation
    p_value_formatted = np.format_float_scientific(p_value, precision=1)
    ionchannels_results.loc[ionchannels_results["metric"] == metric_name, "P-Value"] = (
        p_value_formatted
    )

ionchannels_results["metric"] = ionchannels_results["metric"].apply(
    lambda x: "eval_" + x if not x.startswith("eval_") else x
)


# Ensure the DataFrame is sorted by the metric order before pivoting
ionchannels_results["metric_order"] = ionchannels_results["metric"].map(
    lambda x: key_metrics.index(x)
)
ionchannels_results.sort_values(by="metric_order", inplace=True)

# Pivot the table to get the desired layout
ionchannels_latex_table = ionchannels_results.pivot_table(
    index="metric",
    columns="model",
...
ionchannels_latex_table = ionchannels_latex_table.set_index("metric")
ionchannels_latex_table = ionchannels_latex_table.reindex(
    key_metrics
)  # Reorder based on key_metrics
ionchannels_latex_table.reset_index(inplace=True)
...
for metric in key_metrics:
    ...
    for model in ["Ankh", "TooT-PLM-P2S"]:
        mean_std = ionchannels_results[
            (ionchannels_results["metric"] == metric)
            & (ionchannels_results["model"] == model)
        ]["Mean ± Std"].values[0]
...
for metric in key_metrics:
    metric_name = metric.replace("eval_", "")
    p_value = ionchannels_p_values[metric_name]
    transposed_df.loc["P-Value", metric_name] = p_value

# Convert the DataFrame to LaTeX
latex_code_transposed = transposed_df.to_latex(
    index=True,
    caption="ionchannels Prediction Performance Comparison Transposed with P-Values",
    label="tab:ionchannels_comparison_transposed_p_values",
