...
def compute_p_value(df, task, metric, ssp=None):
    ...
    if len(ankh_values) == len(p2s_values) and len(ankh_values) > 1:
        ...
        p_value_dict[metric.replace("eval_", "")] = np.format_float_scientific(
            p_value, precision=1
        )
    else:
        p_value = np.nan
        p_value_dict[metric.replace("eval_", "")] = np.nan  # Handle NaN case
...
# Add the computed p-value to the fluorescence results DataFrame
fluorescence_table["P-Value"] = fluorescence_table["model"].map(
    lambda model: p_value_fluorescence.get("spearmanr", np.nan)
)

# Format the P-Value in scientific notation only if it's not NaN
# fluorescence_table["P-Value"] = fluorescence_table["P-Value"].apply(
#     lambda x: np.format_float_scientific(x, precision=1) if pd.notnull(x) else "NaN"
# )

# Filter data for Ankh and TooT-PLM-P2S models for eval_spearmanr metric
ankh_fluorescence_data = cv_data[
    (cv_data["task"] == "fluorescence") & (cv_data["model"] == "ankh") & (cv_data["metric"] == "eval_spearmanr")
]
p2s_fluorescence_data = cv_data[
    (cv_data["task"] == "fluorescence") & (cv_data["model"] == "p2s") & (cv_data["metric"] == "eval_spearmanr")
]
