...
# Filter data for the "mp Prediction" task
mp_data = data[data["task"] == "mp"]

# Further filter for cross-validation data split
mp_cv_data = mp_data[
    mp_data["data_split"] == "cross-validation"
]
# Group by model and metric to summarize the performance across folds
mp_summary = (
    mp_cv_data.groupby(["model", "metric"])["value"]
    .agg(["mean", "std"])
    .reset_index()
)
...
mp_summary_filtered = mp_summary[
    mp_summary["metric"].isin(key_metrics)
]
...
# Process data and compute p-values for the "mp Prediction" task
mp_results, mp_p_values = process_and_compare_data(
    mp_cv_data, "mp", key_metrics
)
...
for idx, metric in enumerate(key_metrics):
    ...
    subset = mp_results[mp_results["metric"] == metric_name]
    ...
    ax.text(
        ...
        f"p-value: {mp_p_values.get(metric_name, np.nan)}",
        ha="center",
        va="bottom",
        fontsize=12,
    )
...
# plt.suptitle("mp Prediction Performance Comparison", fontsize=16)
plt.tight_layout(rect=[0, 0.03, 1, 0.95])
# plt.show()

plt.savefig("./plots/plot3.2.6_mp.png", bbox_inches="tight", dpi=300)
plt.close()


# Preparing data for LaTeX table
mp_results["Mean ± Std"] = mp_results.apply(
    lambda row: f"{row['mean']:.3f} ± {row['std']:.3f}", axis=1
)
mp_results["P-Value"] = mp_results["metric"].map(mp_p_values)

# Generating LaTeX table
mp_latex_table = mp_results.pivot(
    index="metric", columns="model", values=["Mean ± Std", "P-Value"]
).reset_index()
# Flatten the multi-level column index
mp_latex_table.columns = [
    " ".join(col).strip() for col in mp_latex_table.columns.values
]

# Print current column names to verify the structure
print("Current column names:", mp_latex_table.columns.tolist())
...
mp_latex_table.columns = new_column_names

# Initialize a list to store the p-values for each metric
p_values = []

# Add p-values to the mp_results DataFrame
for metric in key_metrics:
    metric_name = metric.replace("eval_", "")
    p_value = compute_p_value(mp_cv_data, "mp", metric)
    # Format the p-value with one decimal in scientific notation
    p_value_formatted = np.format_float_scientific(p_value, precision=1)
    mp_results.loc[mp_results["metric"] == metric_name, "P-Value"] = (
        p_value_formatted
    )

mp_results["metric"] = mp_results["metric"].apply(
    lambda x: "eval_" + x if not x.startswith("eval_") else x
)


# Ensure the DataFrame is sorted by the metric order before pivoting
mp_results["metric_order"] = mp_results["metric"].map(
    lambda x: key_metrics.index(x)
)
mp_results.sort_values(by="metric_order", inplace=True)

# Pivot the table to get the desired layout
mp_latex_table = mp_results.pivot_table(
    index="metric",
    columns="model",
...
mp_latex_table = mp_latex_table.set_index("metric")
mp_latex_table = mp_latex_table.reindex(
    key_metrics
)  # Reorder based on key_metrics
mp_latex_table.reset_index(inplace=True)
...
for metric in key_metrics:
    ...
    for model in ["Ankh", "TooT-PLM-P2S"]:
        mean_std = mp_results[
            (mp_results["metric"] == metric)
            & (mp_results["model"] == model)
        ]["Mean ± Std"].values[0]
...
for metric in key_metrics:
    metric_name = metric.replace("eval_", "")
    p_value = mp_p_values[metric_name]
    transposed_df.loc["P-Value", metric_name] = p_value

# Convert the DataFrame to LaTeX
latex_code_transposed = transposed_df.to_latex(
    index=True,
    caption="Membrane Proteins Prediction Performance Comparison Transposed with P-Values",
    label="tab:mp_comparison_transposed_p_values",
