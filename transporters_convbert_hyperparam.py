...
def load_data():
    return load_embeddings_and_labels("./embeddings/p2s_transporters", "transporters")
...
def create_datasets(
    ...
    class TransportersDataset(Dataset):
        ...
        def __getitem__(self, idx):
            ...
            label_int = torch.tensor(label_int, dtype=torch.float).unsqueeze(-1)
    ...
    full_training_dataset = TransportersDataset(training_sequences, training_labels)
    ...
    test_dataset = TransportersDataset(test_sequences, test_labels)
...
def main():
    model_type = "p2s"
    experiment = f"transporters_hyperparam_{model_type}"
