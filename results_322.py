...
# Filter data for the "Solubility Prediction" task
solubility_data = data[data["task"] == "solubility"]

# Further filter for cross-validation data split
solubility_cv_data = solubility_data[
    solubility_data["data_split"] == "cross-validation"
]
# Group by model and metric to summarize the performance across folds
solubility_summary = (
    solubility_cv_data.groupby(["model", "metric"])["value"]
    .agg(["mean", "std"])
    .reset_index()
)
...
solubility_summary_filtered = solubility_summary[
    solubility_summary["metric"].isin(key_metrics)
]
...
# Process data and compute p-values for the "Solubility Prediction" task
solubility_results, solubility_p_values = process_and_compare_data(
    solubility_cv_data, "solubility", key_metrics
)
...
for idx, metric in enumerate(key_metrics):
    ...
    subset = solubility_results[solubility_results["metric"] == metric_name]
    ...
    ax.text(
        ...
        f"p-value: {solubility_p_values.get(metric_name, np.nan)}",
        ha="center",
        va="bottom",
        fontsize=12,
    )
...
# plt.suptitle("Solubility Prediction Performance Comparison", fontsize=16)
plt.tight_layout(rect=[0, 0.03, 1, 0.95])
# plt.show()

plt.savefig("./plots/plot3.2.2_sol.png", bbox_inches="tight", dpi=300)
plt.close()
...
solubility_latex_table = solubility_results.pivot(
    index="metric", columns="model", values=["Mean ± Std", "P-Value"]
).reset_index()
# Flatten the multi-level column index
solubility_latex_table.columns = [
    " ".join(col).strip() for col in solubility_latex_table.columns.values
]

# Print current column names to verify the structure
print("Current column names:", solubility_latex_table.columns.tolist())
...
solubility_latex_table.columns = new_column_names

# Initialize a list to store the p-values for each metric
p_values = []

# Add p-values to the solubility_results DataFrame
for metric in key_metrics:
    metric_name = metric.replace("eval_", "")
    p_value = compute_p_value(solubility_cv_data, "solubility", metric)
    # Format the p-value with one decimal in scientific notation
    p_value_formatted = np.format_float_scientific(p_value, precision=1)
    solubility_results.loc[solubility_results["metric"] == metric_name, "P-Value"] = (
        p_value_formatted
    )
    
solubility_results['metric'] = solubility_results['metric'].apply(lambda x: 'eval_' + x if not x.startswith('eval_') else x)


# Ensure the DataFrame is sorted by the metric order before pivoting
solubility_results['metric_order'] = solubility_results['metric'].map(lambda x: key_metrics.index(x))
solubility_results.sort_values(by='metric_order', inplace=True)

# Pivot the table to get the desired layout
solubility_latex_table = solubility_results.pivot_table(
    index='metric', 
    columns='model', 
...
solubility_latex_table.columns = [' '.join(col).strip() for col in solubility_latex_table.columns.values]
solubility_latex_table = solubility_latex_table.set_index('metric')
solubility_latex_table = solubility_latex_table.reindex(key_metrics)  # Reorder based on key_metrics
solubility_latex_table.reset_index(inplace=True)

# Initialize the transposed DataFrame
transposed_df = pd.DataFrame(index=['Ankh', 'TooT-PLM-P2S', 'P-Value'], columns=[m.replace('eval_', '') for m in key_metrics])

# Fill in the mean ± std values
for metric in key_metrics:
    metric_name = metric.replace('eval_', '')
    for model in ['Ankh', 'TooT-PLM-P2S']:
        mean_std = solubility_results[(solubility_results['metric'] == metric) & (solubility_results['model'] == model)]['Mean ± Std'].values[0]
        transposed_df.loc[model, metric_name] = mean_std

# Add p-values
for metric in key_metrics:
    metric_name = metric.replace('eval_', '')
    p_value = solubility_p_values[metric_name]
